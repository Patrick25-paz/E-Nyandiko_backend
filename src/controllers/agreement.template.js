...
function renderTermsSection(agreement, termsObj) {
  ...
  if (termsObj.transactionType === 'BUY' && termsObj.topUpPlan) {
    html += renderTermRow('Payment Plan', safeString(termsObj.topUpPlan));
  }
...
<body>
  <div class="sheet">
    ...
    <div class="footer">
      ...
      <div class="footer-meta">${escapeHtml(agreement.id)}<br/>Generated ${escapeHtml(new Date().toLocaleString(locale))}</div>
    </div>
  </div>
</body>
</html>`;
